...
function select($where){
  $query = mysql_query("SELECT a.*, b.*, c.kategori_name,d.item_price, e.branch_name FROM item_stocks a
                        LEFT JOIN items b ON b.item_id = a.item_id
                        LEFT JOIN kategori c ON c.kategori_id = b.kategori_id
...
function read_id($id,$branch_id){
  $query = mysql_query("SELECT a.*, b.*, c.item_type_name, d.kategori_name, e.branch_name FROM item_stocks a
                        JOIN items b ON b.item_id = a.item_id
                        LEFT JOIN items_types c ON c.item_type_id = b.item_type
                        LEFT JOIN kategori d ON d.kategori_id = b.kategori_id
                        LEFT JOIN branches e on e.branch_id =a.branch_id
