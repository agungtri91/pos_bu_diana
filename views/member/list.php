<!-- MEMBER -->
<?php if(isset($_GET['did']) && $_GET['did'] == 1){ ?>
<section class="content_new">
  <div class="alert alert-info alert-dismissable">
    <i class="fa fa-check"></i>
    <button class="close" aria-hidden="true" data-dismiss="alert" type="button">×</button>
    <b>Sukses !</b>
    Simpan Berhasil
  </div>
</section>
<?php }else if(isset($_GET['did']) && $_GET['did'] == 2){ ?>
<section class="content_new">
  <div class="alert alert-info alert-dismissable">
    <i class="fa fa-check"></i>
    <button class="close" aria-hidden="true" data-dismiss="alert" type="button">×</button>
    <b>Sukses !</b>
    Edit Berhasil
  </div>
</section>
<?php }else if(isset($_GET['did']) && $_GET['did'] == 3){ ?>
<section class="content_new">
  <div class="alert alert-info alert-dismissable">
    <i class="fa fa-check"></i>
    <button class="close" aria-hidden="true" data-dismiss="alert" type="button">×</button>
    <b>Sukses !</b>
    Delete Berhasil
  </div>
</section>
<?php } ?>
<!-- Main content -->
<section class="content">
  <div class="row">
    <div class="col-xs-12">
      <div class="title_page"> <?= $title ?></div>
      <div class="box">
        <div class="box-body2 table-responsive">
          <table id="example1" class="table table-bordered table-striped">
            <thead>
              <tr>
                <th style="text-align:center;" width="5%">No</th>
                <th style="text-align:center;">Nama </th>
                <th style="text-align:center;">Telepon </th>
                <th style="text-align:center;">Email</th>
                <th style="text-align:center;">Ket.</th>
                <th style="text-align:center;">Config</th>
            </tr>
            </thead>
          <tbody>
          <?php
          $no = 1;
          while($row = mysql_fetch_array($query)){ ?>
            <tr>
              <td style="text-align:center;"><?= $no?></td>
              <td><?= $row['member_name']?></td>
              <td><?= $row['member_phone']?></td>
              <td><?= $row['member_email']?></td>
              <td style="text-align:center;"><?= $row["type_pembeli_name"] ? $row["type_pembeli_name"] : "-"?></td>
              <td style="text-align:center;">
                <?php if (strpos($permit, 'r') !== false ): ?>
                  <a type="button" class="btn btn-default" href="member.php?page=history_member&id=<?= $row['member_id']?>">
                    <i class="fa fa-search"></i>
                  </a>
                <?php endif; ?>
                <?php if (strpos($permit, 'c') !== false || strpos($permit, 'u') !== false): ?>
                  <a href="member.php?page=form&id=<?= $row['member_id']?>" class="btn btn-default">
                    <i class="fa fa-pencil"></i>
                  </a>
                <?php endif; ?>
                <?php if (strpos($permit, 'd') !== false): ?>
                  <a href="javascript:void(0)" onclick="confirm_delete(<?= $row['member_id']; ?>,'member.php?page=delete&id=')"
                    class="btn btn-default" >
                    <i class="fa fa-trash-o"></i>
                  </a>
                <?php endif; ?>
              </td>
            </tr>
          <?php $no++; } ?>
          </tbody>
          <tfoot>
            <?php
             if (strpos($permit, 'c') !== false ): ?>
              <tr>
                <td colspan="6"><a href="<?= $add_button ?>" class="btn btn-danger " >Tambah</a></td>
              </tr>
            <?php endif; ?>
          </tfoot>
          </table>
        </div><!-- /.box-body -->
      </div><!-- /.box -->
    </div>
  </div>
</section><!-- /.content -->
