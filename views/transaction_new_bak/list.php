									...
									<div id="table_history"></div>
										...
										<br>
											...
											<div class="form-group">
												...
												<select class="selectpicker show-tick form-control" id="kategori" name="kategori" data-live-search="true" value="0">
													<option value="0">-Pilih Kategori-</option onclick="menu()">
													<?php
														$query=mysql_query("select * from kategori");
														while($m_kategori = mysql_fetch_array($query)){?>
													<option value="<?= $m_kategori['kategori_id']?>"><?= $m_kategori['kategori_name']?></option onclick="menu()">
													<?php }?>
												</select>
											</div>
										</div>
										...
										<div class="col-md-3">
											<div class="form-group">
												...
												<select class="show-tick form-control" id="cb_sub_kategori" name="cb_sub_kategori" data-live-search="true">
										...
										<div class="col-md-3">
											<div class="form-group">
												<label>Item</label>
												<select class="show-tick form-control" id="cb_item" name="cb_item" data-live-search="true" value="0">
										...
										<div class="col-md-3">
											...
											&nbsp<br>
												...
												<button type="button" name="addTocart" id="addTocart" class="btn btn-info">
		...
		<section class="content_checkout">
			<div class="row">
				<div class="col-md-6">
					...
					<div class="col-xs-4">
						<div class="form-group">
							<input type="button" class="btn btn-danger button_checkout" onclick="simpan_form()" value="Simpan"/>
						</div>
					</div>
				</div>
				<div class="col-md-6">
...
<script type="text/javascript">
   ...
   jQuery(document).ready(function(){
   	...
   	getcart2();
   			...
   			$.ajax({
   				...
   				data : {
					...
					success : function(option){
						// $('#cb_sub_kategori').remove();
						$('#cb_sub_kategori').html(option);
					}
   			});
			var transaction_id = <?php echo $transaction_id ?>;
   			var sub = $("#cb_sub_kategori").val();
   			var branch = $('#i_branch_id').val();
   		...
   		$("#cb_sub_kategori").change(function(){
   			var transaction_id = <?php echo $transaction_id ?>;
   			var sub = $("#cb_sub_kategori").val();
   			var branch = $('#i_branch_id').val();
   			var id_kategori = $("#kategori").val();
		...
		function addcart(x){
			...
			// 		if (x==cart[keys[j]]) {
			// 			ada = 1;
			// 			console.log(cart[keys[j]].item_id);
			// 		}
			// 	}
			// }
			...
			if (ada==0) {
				...
				$.ajax({
					type:'POST',
					url:'transaction_new.php?page=cart',
		...
		function hapuscart(id) {
			$.ajax({
					type:'POST',
					url:'transaction_new.php?page=hapuscart',
					data: {id:id},
					dataType:'json',
				}).done(function(data){
					getcart();
		...
		function getcart2() {
			$.ajax({
					type:'POST',
					url:'transaction_new.php?page=getcart',
					dataType:'json',
				}).done(function(data){
					var no = 1;
					$('#list_cart_body_tb').empty();
						console.log(data);
					for(var i = 0; i <= data.length; i++){
						if (data[i].st) {


							$('#list_cart_body_tb').append('\
								<tr id="tr_1_'+i+'">\
									...
									<td style="text-align:center;">\
										<input type="button" id="hapus_'+i+'""\
										 class="btn btn-danger tombol_focus" onclick="hapuscart('+i+')" value="Hapus">\
									</td>\
								</tr>\
							');
